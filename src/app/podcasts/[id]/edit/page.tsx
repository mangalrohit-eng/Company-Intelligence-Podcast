...
export default function EditPodcastPage() {
  ...
  const fetchPodcastData = async () => {
    try {
      ...
      if (response.ok) {
        ...
        toast.error('Failed to Load Podcast', 'Failed to load podcast data');
        router.push('/podcasts');
      }
    } catch (error) {
      console.error('Error fetching podcast:', error);
      toast.error('Error Loading Podcast', 'Error loading podcast data');
      router.push('/podcasts');
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = async () => {
    try {
      ...
      if (response.ok) {
        toast.success('Podcast Updated', 'Podcast updated successfully!');
        setTimeout(() => {
          router.push(`/podcasts/${podcastId}`);
        }, 1000);
      } else {
        const error = await response.text();
        toast.error('Failed to Update Podcast', error);
      }
    } catch (error) {
